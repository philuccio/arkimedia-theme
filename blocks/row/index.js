...
registerBlockType( metadata.name, {
    ...
    edit( { attributes, setAttributes } ) {
        ...
        const gridRowsText  = useLocalText( a.gridRows,   v => setAttributes({ gridRows: v }) )
        const widthText     = useLocalText( a.width,      v => setAttributes({ width: v }) )
        const minHeightText = useLocalText( a.minHeight,  v => setAttributes({ minHeight: v }) )
        const maxWidthText  = useLocalText( a.maxWidth,   v => setAttributes({ maxWidth: v }) )
        const zIndexText    = useLocalText( a.zIndex,     v => setAttributes({ zIndex: v }) )
        const marginTopText    = useLocalText( a.marginTop,    v => setAttributes({ marginTop: v }) )
        const marginRightText  = useLocalText( a.marginRight,  v => setAttributes({ marginRight: v }) )
        const marginBottomText = useLocalText( a.marginBottom, v => setAttributes({ marginBottom: v }) )
        const marginLeftText   = useLocalText( a.marginLeft,   v => setAttributes({ marginLeft: v }) )
                ...
                <InspectorControls>
                    ...
                    <PanelBody title={ __( 'Generali', 'arkimedia' ) } initialOpen={true}>
                        <SelectControl label={ __('Tag HTML','arkimedia') } value={a.htmlTag}
                            options={[
                                { label:'div', value:'div' }, { label:'section', value:'section' },
                                { label:'article', value:'article' }, { label:'main', value:'main' },
                                { label:'aside', value:'aside' }, { label:'header', value:'header' },
                                { label:'footer', value:'footer' }, { label:'nav', value:'nav' },
                            ]}
                            onChange={ v => setAttributes({ htmlTag: v }) }
                        />
                        <ToggleControl label={ __('Fullwidth (100vw)','arkimedia') } checked={a.isFullwidth} onChange={ v => setAttributes({ isFullwidth: v }) } />
                    </PanelBody>
                    ...
                    <PanelBody title={ __( 'Layout', 'arkimedia' ) } initialOpen={true}>
                        ...
                        { a.display === 'flex' && <>
                            <SelectControl label={ __('Direzione','arkimedia') } value={a.flexDirection}
                                options={[ { label:'Row →', value:'row' }, { label:'Row ←', value:'row-reverse' }, { label:'Column ↓', value:'column' }, { label:'Column ↑', value:'column-reverse' } ]}
                                onChange={ v => setAttributes({ flexDirection: v }) }
                            />
                            <SelectControl label={ __('Flex wrap','arkimedia') } value={a.flexWrap}
                                options={[ { label:'Wrap', value:'wrap' }, { label:'No wrap', value:'nowrap' }, { label:'Wrap ↑', value:'wrap-reverse' } ]}
                                onChange={ v => setAttributes({ flexWrap: v }) }
                            />
                            <SelectControl label={ __('Justify content','arkimedia') } value={a.justifyContent}
                                options={[ { label:'Start', value:'flex-start' }, { label:'Center', value:'center' }, { label:'End', value:'flex-end' }, { label:'Space between', value:'space-between' }, { label:'Space around', value:'space-around' }, { label:'Space evenly', value:'space-evenly' } ]}
                                onChange={ v => setAttributes({ justifyContent: v }) }
                            />
                            <SelectControl label={ __('Align items','arkimedia') } value={a.alignItems}
                                options={[ { label:'Stretch', value:'stretch' }, { label:'Start', value:'flex-start' }, { label:'Center', value:'center' }, { label:'End', value:'flex-end' }, { label:'Baseline', value:'baseline' } ]}
                                onChange={ v => setAttributes({ alignItems: v }) }
                            />
                            <SelectControl label={ __('Align content','arkimedia') } value={a.alignContent}
                                options={[ { label:'Normal', value:'normal' }, { label:'Start', value:'flex-start' }, { label:'Center', value:'center' }, { label:'End', value:'flex-end' }, { label:'Space between', value:'space-between' }, { label:'Space around', value:'space-around' }, { label:'Stretch', value:'stretch' } ]}
                                onChange={ v => setAttributes({ alignContent: v }) }
                            />
                        </> }

                        { a.display === 'grid' && <>
                            <LocalRange label={ __('Colonne','arkimedia') } value={a.gridColumns} onChange={ v => setAttributes({ gridColumns: v }) } min={1} max={12} />
                            <TextControl label={ __('Grid rows','arkimedia') } { ...gridRowsText } />
                            <SelectControl label={ __('Auto flow','arkimedia') } value={a.gridAutoFlow}
                                options={[ { label:'Row', value:'row' }, { label:'Column', value:'column' }, { label:'Dense', value:'dense' } ]}
                        ...
                        { a.useCustomGap ? <>
                            <LocalRange label="Column gap (px)" value={a.columnGap} onChange={ v => setAttributes({ columnGap: v }) } max={100} />
                            <LocalRange label="Row gap (px)"    value={a.rowGap}    onChange={ v => setAttributes({ rowGap: v }) }    max={100} />
                        </> : <LocalRange label="Gap (px)" value={a.gap} onChange={ v => setAttributes({ gap: v }) } max={100} /> }
                    </PanelBody>

                    <PanelBody title={ __( 'Dimensioni', 'arkimedia' ) } initialOpen={false}>
                        { ! a.isFullwidth && <TextControl label={ __('Width','arkimedia') } { ...widthText } /> }
                        <TextControl label={ __('Min height','arkimedia') } { ...minHeightText } placeholder="es. 400px" />
                        <TextControl label={ __('Max width','arkimedia') }  { ...maxWidthText }  placeholder="es. 1200px" />
                        <SelectControl label={ __('Overflow','arkimedia') } value={a.overflow}
                            options={[ { label:'Visible', value:'visible' }, { label:'Hidden', value:'hidden' }, { label:'Auto', value:'auto' } ]}
                            onChange={ v => setAttributes({ overflow: v }) }
                        />
                        <TextControl label={ __('Z-index','arkimedia') } { ...zIndexText } type="number" />
                    </PanelBody>

                    <PanelBody title={ __( 'Padding', 'arkimedia' ) } initialOpen={false}>
                        <LocalRange label="Top"    value={a.paddingTop}    onChange={ v => setAttributes({ paddingTop: v }) } />
                        <LocalRange label="Right"  value={a.paddingRight}  onChange={ v => setAttributes({ paddingRight: v }) } />
                        <LocalRange label="Bottom" value={a.paddingBottom} onChange={ v => setAttributes({ paddingBottom: v }) } />
                        <LocalRange label="Left"   value={a.paddingLeft}   onChange={ v => setAttributes({ paddingLeft: v }) } />
                    </PanelBody>

                    <PanelBody title={ __( 'Margin', 'arkimedia' ) } initialOpen={false}>
                        <TextControl label="Top"    { ...marginTopText }    placeholder="0 / auto" />
                        <TextControl label="Right"  { ...marginRightText }  placeholder="0 / auto" />
                        <TextControl label="Bottom" { ...marginBottomText } placeholder="0 / auto" />
                        <TextControl label="Left"   { ...marginLeftText }   placeholder="0 / auto" />
                    </PanelBody>
                    ...
                    <PanelBody title={ __( 'Sfondo', 'arkimedia' ) } initialOpen={false}>
                        ...
                        { a.bgType === 'gradient' && <>
                            ...
                            />
                            { a.gradientType === 'linear' && <LocalRange label="Angolo (°)" value={a.gradientAngle} onChange={ v => setAttributes({ gradientAngle: v }) } min={0} max={360} /> }
                            ...
                            <LocalRange label="Posizione 1 (%)" value={a.gradientPos1} onChange={ v => setAttributes({ gradientPos1: v }) } min={0} max={100} />
                            ...
                            <LocalRange label="Posizione 2 (%)" value={a.gradientPos2} onChange={ v => setAttributes({ gradientPos2: v }) } min={0} max={100} />
                        </> }
                        { a.bgType === 'image' && <>
                            <MediaUploadCheck>
                                <MediaUpload
                            ...
                            <SelectControl label="Position" value={a.bgPosition}
                                options={[ { label:'Center', value:'center center' }, { label:'Top', value:'center top' }, { label:'Bottom', value:'center bottom' }, { label:'Left', value:'left center' }, { label:'Right', value:'right center' } ]}
                    ...
                    <PanelBody title={ __( 'Bordo', 'arkimedia' ) } initialOpen={false}>
                        ...
                        <LocalRange label="Top"    value={a.borderTopWidth}    onChange={ v => setAttributes({ borderTopWidth: v }) }    max={20} />
                        <LocalRange label="Right"  value={a.borderRightWidth}  onChange={ v => setAttributes({ borderRightWidth: v }) }  max={20} />
                        <LocalRange label="Bottom" value={a.borderBottomWidth} onChange={ v => setAttributes({ borderBottomWidth: v }) } max={20} />
                        <LocalRange label="Left"   value={a.borderLeftWidth}   onChange={ v => setAttributes({ borderLeftWidth: v }) }   max={20} />
                        ...
                        <LocalRange label="Top Left"     value={a.borderRadiusTL} onChange={ v => setAttributes({ borderRadiusTL: v }) } />
                        <LocalRange label="Top Right"    value={a.borderRadiusTR} onChange={ v => setAttributes({ borderRadiusTR: v }) } />
                        <LocalRange label="Bottom Right" value={a.borderRadiusBR} onChange={ v => setAttributes({ borderRadiusBR: v }) } />
                        <LocalRange label="Bottom Left"  value={a.borderRadiusBL} onChange={ v => setAttributes({ borderRadiusBL: v }) } />
                    </PanelBody>

                    <PanelBody title={ __( 'Effetti', 'arkimedia' ) } initialOpen={false}>
                        <LocalRange label={ __('Opacità','arkimedia') } value={a.opacity} onChange={ v => setAttributes({ opacity: v }) } min={0} max={1} step={0.05} />
                        <ToggleControl label={ __('Box shadow','arkimedia') } checked={a.boxShadow} onChange={ v => setAttributes({ boxShadow: v }) } />
                        { a.boxShadow && <>
                            <LocalRange label="Offset X" value={a.shadowOffsetX} onChange={ v => setAttributes({ shadowOffsetX: v }) } min={-50} max={50} />
                            <LocalRange label="Offset Y" value={a.shadowOffsetY} onChange={ v => setAttributes({ shadowOffsetY: v }) } min={-50} max={50} />
                            <LocalRange label="Blur"     value={a.shadowBlur}    onChange={ v => setAttributes({ shadowBlur: v }) }    min={0}   max={100} />
                            <LocalRange label="Spread"   value={a.shadowSpread}  onChange={ v => setAttributes({ shadowSpread: v }) }  min={-50} max={50} />
                        ...
                        <LocalRange label="Translate X (px)" value={a.translateX} onChange={ v => setAttributes({ translateX: v }) } min={-200} max={200} />
                        <LocalRange label="Translate Y (px)" value={a.translateY} onChange={ v => setAttributes({ translateY: v }) } min={-200} max={200} />
                        <LocalRange label="Rotate (deg)"     value={a.rotate}     onChange={ v => setAttributes({ rotate: v }) }     min={-180} max={180} />
                        <LocalRange label="Scale"            value={a.scale}      onChange={ v => setAttributes({ scale: v }) }      min={0.1}  max={3}   step={0.05} />
                    </PanelBody>
                    ...
                    <PanelBody title={ __( 'Animazioni GSAP', 'arkimedia' ) } initialOpen={false}>
                        <SelectControl label={ __('Tipo','arkimedia') } value={a.animationType}
                            options={[ { label:'Nessuna', value:'none' }, { label:'Fade up', value:'fadeUp' }, { label:'Fade in', value:'fadeIn' }, { label:'Slide left', value:'slideLeft' }, { label:'Slide right', value:'slideRight' }, { label:'Scale up', value:'scaleUp' }, { label:'Flip', value:'flip' } ]}
                        ...
                        { a.animationType !== 'none' && <>
                            ...
                            />
                            <LocalRange label={ __('Durata (ms)','arkimedia') }  value={a.animationDuration} onChange={ v => setAttributes({ animationDuration: v }) } min={100} max={3000} step={50} />
                            <LocalRange label={ __('Delay (ms)','arkimedia') }   value={a.animationDelay}    onChange={ v => setAttributes({ animationDelay: v }) }    min={0}   max={2000} step={50} />
                    ...
                    <PanelBody title={ __( 'Responsive — Tablet (≤1024px)', 'arkimedia' ) } initialOpen={false}>
                        ...
                        />
                        <LocalRange label="Gap (px)" value={a.mdGap} onChange={ v => setAttributes({ mdGap: v }) } />
                        <LocalRange label={ __('Colonne grid','arkimedia') } value={a.mdGridColumns} onChange={ v => setAttributes({ mdGridColumns: v }) } min={0} max={12} />
                    </PanelBody>
                    ...
                    <PanelBody title={ __( 'Responsive — Mobile (≤640px)', 'arkimedia' ) } initialOpen={false}>
                        ...
                        />
                        <LocalRange label="Gap (px)" value={a.smGap} onChange={ v => setAttributes({ smGap: v }) } />
                        <LocalRange label={ __('Colonne grid','arkimedia') } value={a.smGridColumns} onChange={ v => setAttributes({ smGridColumns: v }) } min={0} max={12} />
                    </PanelBody>
